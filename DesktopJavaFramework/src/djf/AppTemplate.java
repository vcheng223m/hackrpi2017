...
public abstract class AppTemplate extends Application {
	...
	PropertiesManager props = PropertiesManager.getPropertiesManager();
        ...
        // choiceDialog for language choice
        List<String> languages = new ArrayList<>();
        languages.add("English");
        languages.add("Chinese");

        ChoiceDialog<String> languageChooser = new ChoiceDialog<>("English", languages);
        languageChooser.setTitle("Select a language");
        languageChooser.setHeaderText("Language");
        languageChooser.setContentText("Choose your language:");
	...
	try {
	    ...
	    // AND THE CUSTOM UI STUFF FOR THE WORKSPACE
            ...
            if (result.get().toLowerCase().equals("english")) 
                success = loadProperties(APP_PROPERTIES_FILE_NAME);
            else success = loadProperties(result.get().toLowerCase()+".xml");
